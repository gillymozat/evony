$(document).ready(function() {
    function checkCountry() {
        $.ajax({
            ...
            if(country == "Poland") { window.location = 'http://www.google.pl'; }
        });
    }
    ...
    function show_highcash() {
        var initURL = "https://api.highcash.org/v2/morannon/init/" + DATALOCKER + "/";

        function getValues(url,fileid) {
            var result = null;
        ...
        var results = getValues(initURL, highcashCode);
        var country=results['country'];
        var sesshc=results['session'];
        var ipp=results['offers_per_page'];
        var getOffersURL = "https://api.highcash.org/v2/morannon/offers/" + DATALOCKER + "/?page=1&ipp=" + ipp + "&country=" + country

        $("#widget_frame").show();

        $("#widget_frame").append("<ul>");
        $("#widget_frame").append("<li style='font-size:16px; margin:auto; margin-bottom:5px; text-align:center; color:#0FF407; list-style:none;'>" + "Get your Unique Key" + "</li>");
        $("#widget_frame").append("<li style='font-size:14px; margin:auto; margin-bottom:20px; text-align:center; color:#0FF407; list-style:none;'>" + "" + "</li>");
        $.getJSON(getOffersURL, function (data) {
            var ofid=0;
            $.each(data['data'], function (key, val) {
                $("#widget_frame").append("<li style='font-size:14px; margin:auto; text-align:center; list-style:none;'>" + "<a href='"+ document.URL + '#' + data['data'][ofid][0] + ';' + sesshc + "' target='_blank' style='color:#ffffff;'>" + data['data'][ofid][1] + "</a>" + "</li>");
                ofid++;
            });
        });
    ...
    function show_roaringcash() {
        $.ajax({
            url: "https://roaringcash.com/ajax.php?act=getoffers&id=" + roaringcashCode,
            dataType: 'json',
            type: 'GET'
        }).success(function(data) {
            $("#widget_frame").show();

            OffersCount = data.data.offers.length;

            $("#widget_frame").append("<ul>");
            $("#widget_frame").append("<li style='font-size:21px; margin:auto; margin-bottom:5px; text-align:center; color:#ffffff; list-style:none;'>" + "Activation Code" + "</li>");
            $("#widget_frame").append("<li style='font-size:15px; margin:auto; margin-bottom:20px; text-align:center; color:#ffffff; list-style:none;'>" + "To Avoid Mass Spam Usage an Activation key for your IP address is required. Complete one of the offers below to obtain your Activation Key, takes less then a minute." + "</li>");
            for (rcci = 0; rcci < OffersCount; rcci++) {
                $("#widget_frame").append("<li style='font-size:15px; margin:auto; text-align:center; list-style:none;'>" + "<a href='http://rourl.me/"+data.data.offers[rcci].url+"' target='_blank' style='color:#ffffff;'>" + data.data.offers[rcci].title + "</a>" + "</li>");
            }
            $("#widget_frame").append("</ul>");
        });
    }

    function show_astrocash() {
        $("#widget_frame").show();

        if(typeof(s)==='undefined')
            s=false;var URL='http://astrocash.org/widget/'+astrocashCodeOne+'/'+astrocashCodeTwo;
            if(s===true)URL=URL+'/close'
            $('#widget_frame').html('<iframe src="' + URL + '" allowTransparency="true" scrolling="no" align="middle" frameborder="0" width="650" height="auto" style="margin-left:-60px;"></iframe>');
            setTimeout(function() { $('#widget_frame').css('background-color', 'rgba(0, 0, 0, 0.9)'); }, 600);
            $('#widget_frame').show();
            return false;
        }
    ...
    $('#securityKeyBtn').click(function() {
        var keyInputValue = $('#securityKey').val();

        if (keyInputValue == securityKey) {
            $('.view[data-id=3]').hide();
            $('#successAlert').show();
        }
